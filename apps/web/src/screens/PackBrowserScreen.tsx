...
function PackCard({ pack, onLike }: { pack: CommunityPack; onLike: (id: string) => void }) {
  return (
    ...
    >
      <GlassCard className="rounded-3xl p-5 border border-white/10 hover:border-accent/40 transition">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <h3 className="font-display text-lg font-bold text-white">{pack.title}</h3>
              {pack.isCustom && (
                <span className="rounded-full bg-accent/20 px-2.5 py-0.5 text-[10px] font-semibold text-accent uppercase tracking-wider">
                  My Pack
                </span>
              )}
            </div>
...
export default function PackBrowserScreen() {
  ...
  async function fetchPacks() {
    setLoading(true)
    const combinedPacks: CommunityPack[] = []

    // Load local custom packs
    try {
      const stored = localStorage.getItem('wordmask_custom_packs')
      if (stored) {
        const localList = JSON.parse(stored) as Record<string, unknown>[]
        const customPacks: CommunityPack[] = localList.map((p) => ({
          id: String(p['id'] || crypto.randomUUID()),
          title: String(p['title'] || 'Custom Pack'),
          description: String(p['description'] || 'Uploaded custom word pack.'),
        ...
        combinedPacks.push(...customPacks)
      }
    } catch {
      // Ignore local storage parse errors
    }

    // Load Supabase published community packs
    if (isSupabaseConfigured) {
      try {
        const client = await ensureAnonymousSession()
        const { data } = await client
          .from('community_packs')
          .select('id, title, description, language, category, tags, downloads, likes')
          .eq('status', 'published')
          .order('likes', { ascending: false })
          .limit(50)

        if (data) {
          const remotePacks: CommunityPack[] = data.map((row: Record<string, unknown>) => ({
            id: String(row['id'] ?? ''),
          ...
          combinedPacks.push(...remotePacks)
        }
      } catch {
    ...
    setPacks(combinedPacks)
    setLoading(false)
  }

  function handleLike(packId: string) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center gap-6 overflow-y-auto px-6 py-12">
      ...
      >
        ...
        <p className="mt-2 text-white/50">Browse community packs or create your own with JSON upload.</p>
      </motion.div>
      ...
      <div className="w-full max-w-2xl flex gap-3">
        <input
          ...
          placeholder="Search packs by title, category, tag…"
          className="flex-1 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-white outline-none transition focus:border-accent placeholder:text-white/30"
        />
      ...
      {loading && <p className="text-white/40">Loading packs…</p>}
      ...
      <div className="flex w-full max-w-2xl flex-col gap-4">
        ...
        {!loading && filteredPacks.length === 0 && (
          <div className="rounded-3xl border border-dashed border-white/10 p-12 text-center text-white/40">
            No packs found. Click "+ Create / Upload" to add or upload your JSON pack!
          </div>
        )}
      </div>
